...
export class BestScore extends BaseComponent {
  ...
  createScoreLine(data: IRecord) {
    ...
    const fullName = new BaseComponent('span', ['score__full-name'], `${data.firstName} ${data.lastName}`);
    const emailBox = new BaseComponent('span', ['score__email'], `${data.email}`);
    ...
    const scoreTextBox = new BaseComponent('span', ['score__score-text-box'], `Score: `);
    const scoreBox = new BaseComponent('span', ['score__score-box'], `${data.score || 0}`);
    render(scoreWrap.element, [scoreTextBox.element, scoreBox.element]);

    render(lineBox.element, [
      userBox.element,
      scoreWrap.element
    ]);
