...
export class IndexedDB {
  ...
  constructor() {
    ...
    this.openRequest.onupgradeneeded = () => {
      ...
      if (!this.db.objectStoreNames.contains('players')) {
        const objectStore = this.db.createObjectStore('players', {keyPath: 'email'});
      }
    }
  ...
  addUser(data: IData) { //data must be started by key
    ...
    request.onerror = () => {
      let message = new Modal('Warning!', new Message('Something went wrong. Try again later!'));
    ...
    request.onsuccess = () => {
      let message = new Modal('Successful!', new Message('New player created!'));
      appContainer.appendChild(message.element);
      setTimeout(() => {
        message.element.remove();
      },MESSAGE_TIME)
